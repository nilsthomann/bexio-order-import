...
4. **Season Code**: Enter an optional season prefix (e.g. `FS27`) used to prefix or filter article codes during Bexio article resolution.
5. **Position Text Template**: Define the template for the text written to Bexio order positions (default: `Color: {Color}, Size: {Size}`). Supported placeholders are `{Color}`, `{Size}`, `{ArticleNumber}`, `{ArticleName}`, `{BexioArticleName}`, and `{BexioArticleDescription}`. HTML formatting (e.g. `<strong>`, `<br />`) is fully supported by Bexio. You can configure this template within individual mapping profiles.
